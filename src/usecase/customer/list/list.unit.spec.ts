...
import ListCustomerUseCase from "./list";
...
describe("List Customers use case unit tests", () => {
  it("should list customers", async () => {
    ...
    expect(output.customers[0].address.street).toBe(customer1.street);

    expect(output.customers[1].id).toBe(customer2.id);
    expect(output.customers[1].name).toBe(customer2.name);
    expect(output.customers[1].address.street).toBe(customer2.street);
  })
})